#!/usr/bin/env python3
"""
Experiment 1: Run all 1534 questions with complete evaluation logic
"""
...
from src.graph_agent import app  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 1 full runner (all 1534 questions)")
    parser.add_argument("--out-dir", type=str, default="", help="Output directory (default results/exp1_full_YYYYmmdd_HHMMSS）")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join("results", f"exp1_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    _ensure_dir(out_dir)
    ...
    print(f"🚀 Start running Exp1 full experiment (Questions{start}-{end-1}，total{limit}）")
    ...
    print("-" * 60)
    print(f"🎉 Exp1 full experiment completed! Results saved in: {out_dir}")
