#!/usr/bin/env python3
"""
One-Shot Experiment: Using Aliyun GLM4.7 model, each case includes 1 example
Question range: 0-1534 (all questions)
"""
...
from src.graph_agent_one_shot import build_workflow
...
def _collect_final_state_from_stream(initial_state: dict) -> dict:
    ...
    try:
        ...
        raise TimeoutError("Workflow execution timeout (600s)")
    finally:
        if timer:
            timer.cancel()
...
def main():
    parser = argparse.ArgumentParser(description="One-Shot Experiment: Aliyun GLM-4.7")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join("results", "exp_one_shot")
    _ensure_dir(out_dir)
    ...
    print(f"[Start] Aliyun GLM-4.7 One-Shot：Questions{start}-{end}，total{limit}")
