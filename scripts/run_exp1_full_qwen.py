#!/usr/bin/env python3
"""
Experiment 1: Run all 1534 questions using Qwen3 model
"""
...
from src.graph_agent_qwen import app  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 1 full runner with Qwen3 model")
    parser.add_argument("--out-dir", type=str, default="", help="Output directory (default results/exp1_full_qwen_YYYYmmdd_HHMMSS）")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join("results", f"exp1_full_qwen_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    _ensure_dir(out_dir)
    ...
    print(f"🚀 Start running Exp1 full experiment (Qwen3 model, Questions{start}-{end-1}，total{limit}）")
        ...
        for idx, item in enumerate(slice_items, start=start):
            ...
            try:
                ...
                try:
                    ...
                    record["error"] = "Workflow execution timeout (90s)"
                    f_out.write(json.dumps(record, ensure_ascii=False) + "\n")
                    f_out.flush()
    ...
    print("-" * 60)
    print(f"🎉 Exp1 full experiment (Qwen3 model) completed! Results saved in: {out_dir}")
