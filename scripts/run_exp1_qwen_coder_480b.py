#!/usr/bin/env python3
"""
Experiment 1: Run all 1534 questions using Qwen3-Coder-480B model
"""
...
from src.graph_agent_qwen_coder_480b import app  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 1 full runner with Qwen3-Coder-480B model")
    parser.add_argument("--out-dir", type=str, default="", help="Output directory (default results/exp1_full_qwen3_coder_480b_YYYYmmdd_HHMMSS）")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join("results", f"exp1_full_qwen3_coder_480b_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    _ensure_dir(out_dir)
    ...
    meta = {
        "experiment": "exp1_full_qwen3_coder_480b",
    ...
    print(f"🚀 Start running Exp1 full experiment (Qwen3-Coder-480B model, Questions{start}-{end-1}，total{limit}）")
    ...
    print("-" * 60)
    print(f"🎉 Exp1 full experiment (Qwen3-Coder-480B model) completed! Results saved in: {out_dir}")
