...
# -*- coding: utf-8 -*-
"""
Baseline B Experiment: Monolithic Text-to-DDL/DML (Complete repair+query in one pass)
...
4. Same backbone, same token budget, same self-correction limit
...
from src.graph_agent_baseline_b import app
...
def _collect_final_state_from_stream(initial_state: dict, timeout: float = 600.0) -> dict:
    merged: dict = {}
    timed_out = False
    ...
    worker_thread = threading.Thread(target=_run_workflow)
    worker_thread.daemon = True
    worker_thread.start()
    worker_thread.join(timeout=timeout)

    if worker_thread.is_alive():
        raise TimeoutError(f"Workflow execution timeout ({timeout}s)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Baseline B: Monolithic Text-to-DDL/DML (Complete repair+query in one step)"
    )
    parser.add_argument("--out-dir", type=str, default="", help="Output directory (default results/exp_baseline_b）")
    ...
    if args.out_dir:
        ...
        out_dir = os.path.join("results", "exp_baseline_b")
    _ensure_dir(out_dir)
    ...
    meta = {
        "experiment": "baseline_b_monolithic",
        "description": "Baseline B: Monolithic Text-to-DDL/DML (Complete repair+query in one step). No routing, no skill splitting, single large prompt.",
        "model": "GLM-4.7-thinking (chatanywhere)",
    ...
    print("=" * 70)
    print("[Start] Baseline B: Monolithic Text-to-DDL/DML")
    if args.reverse:
    ...
    print(f"[Feature] No routing, no skill splitting, single large prompt, complete repair+query in one step")
    ...
    with open(results_path, "a", encoding="utf-8") as f_out:
        for idx, item in enumerate(slice_items, start=start):
            ...
            record = {
                ...
                "rsr_diff_summary": "",
                "baseline": "B",
                "baseline_name": "Monolithic Text-to-DDL/DML"
            }
            ...
            try:
                ...
                sandbox_dir = os.path.join("temp_env", "exp_baseline_b")
                ...
                has_recovery_json = os.path.exists(recovery_json_path) and bool(read_json_file(recovery_json_path).get("case_type"))

                ground_truth_case_type = "normal"
                if has_recovery_json:
                    try:
                        recovery_data = read_json_file(recovery_json_path)
                        ground_truth_case_type = recovery_data.get("case_type", "normal")
                    except Exception:
                        pass
                record["ground_truth_case_type"] = ground_truth_case_type
                ...
                try:
                    ...
                    record["case_type"] = final_state.get("case_type", "")
                        ...
                        if gold_sql:
                            try:
                                ...
                                if isinstance(original_gold_result, str) and original_gold_result.startswith("Error:"):
                                    ...
                                    if not has_recovery_json:
                                        record["eval_is_match"] = True
                                        record["eval_diff_summary"] = "NO_REPAIR_NEEDED"
                                        record["rsr_is_match"] = True
                                        record["rsr_diff_summary"] = "NO_REPAIR_NEEDED"
                                    else:
                                        try:
                                            ...
                                            record["rsr_is_match"] = is_match
                                            record["rsr_diff_summary"] = diff_summary
                                        except Exception as eval_e:
                                            record["rsr_is_match"] = False
                                            record["rsr_diff_summary"] = f"RSR_EVAL_ERROR: {repr(eval_e)}"
            ...
            print(f"{status_symbol} Completed questions {idx}: {question_id} ({elapsed:.1f}s, case={record.get('case_type', '')})", flush=True)

            if not args.keep_temp:
                sandbox_db = record.get("db_path", "")
                sandbox_recovery = os.path.join("temp_env", "exp_baseline_b", f"{question_id}_recovery.json") if question_id is not None else ""
                try:
                    if sandbox_db:
                        safe_remove_sandbox(sandbox_db, sandbox_recovery)
                    else:
                        safe_remove_sandbox(
                            os.path.join("temp_env", "exp_baseline_b", f"{question_id}_{db_id}.sqlite"),
                            sandbox_recovery
                        )
                except Exception:
                    pass
    ...
    print("\n" + "=" * 70)
    print(f"[Done] Baseline B experiment completed!")
